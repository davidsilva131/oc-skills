...
> **⚠️ Check whether the project uses Next.js or Vite — the setup differs.**

### If Next.js:
> - Use `@tailwindcss/postcss` (PostCSS plugin), NOT `@tailwindcss/vite`
> - No `vite.config.ts` — skip Vite-specific templates
> - Use Next.js's `tsconfig.json` (not `tsconfig.app.json`)
> - CSS file typically at `app/globals.css` (not `src/index.css`)
> - PostCSS config uses `@tailwindcss/postcss` plugin
>
> The general Tailwind v4 principles (hsl() wrapping, @theme inline, CSS variable architecture) still apply — just skip the Vite-specific steps.

### If Vite:
> - Use `@tailwindcss/vite` plugin in `vite.config.ts`
> - CSS file at `src/index.css`
> - Apply all Vite-specific steps in the templates
