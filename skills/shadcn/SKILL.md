...
> **⚠️ IMPORTANT**: This skill assumes the **standard shadcn/ui "new york" style** components. If your project uses the newer `FieldGroup`/`Field`/`InputGroup` component system, adjust accordingly.

### Typical AVAILABLE components:
- `Button`, `Badge`, `Card` (+ `CardHeader`, `CardTitle`, `CardDescription`, `CardContent`, `CardFooter`, `CardAction`)
- `Input`, `Label`, `Separator`, `Skeleton`, `Avatar`, `Switch`, `Checkbox`
- `Select` (+ all sub-components: `SelectTrigger`, `SelectContent`, `SelectItem`, `SelectGroup`, etc.)
- `Dialog`, `Drawer` (vaul), `Popover`, `DropdownMenu`, `Command` (cmdk)
- `Calendar`, `Pagination`, `Table`, `Toaster` (sonner)

### Often NOT available (verify per project):
- `FieldGroup`, `Field`, `InputGroup`, `FieldSet`, `ToggleGroup`, `Combobox`, `Empty`, `Spinner`
- `Alert`, `Textarea` (— use native `<textarea>` instead)
